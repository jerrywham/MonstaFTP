...
function processForm(vars) {
	...
	vars = vars + "&windowWidth=" + document.body.offsetWidth
	vars = vars + "&windowHeight=" + document.body.offsetHeight
...
function setDragFile(e,file,folder) {
...
function dropFile(folder) {
	...
	var vars = '&ftpAction=dragDrop&dragFile='+globalDragFile+'&dropFolder='+encodeURIComponent(folder);
	processForm(vars);
	activateActionButtons(0,0);	
}
...
function displayUploadButtons() {
	...
	if (globalBrowser == "ie9-") {
		...
		html += '<input type="button" value="'+lang_btn_upload_file+'" onClick="uploadFileCheck(event)">';
	}
	
	// Non-IE9 Upload File
	if (globalBrowser != "ie9-") {
		html += '<input type="button" class="inputButton" value="'+lang_btn_upload_files+'" onClick="fileChoose(event)">';
	}
	...
	html += '<div id="repeatUploadDiv" style="visibility: hidden; display: none">';
		html += '<div class="floatLeft10">';
			html += '<input type="button" class="inputButton" value="'+lang_btn_upload_repeat+'" onclick="processFileUploads(event,0,0,\'\')">';
		html += '</div>';
	html += '</div>';
	...
	if (globalBrowser == "chrome") {
		html += '<div class="floatLeft">';
			html += '<input type="button" class="inputButton" value="'+lang_btn_upload_folder+'" onClick="dirChoose(event)">';
